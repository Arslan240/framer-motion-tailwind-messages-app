...
export default function Email() {
  ...
  function toggleMessage(mid) {
    if (selectedMessages.includes(mid)) {
      setSelectedMessages((selectedMessages) => selectedMessages.filter((id) => id !== mid))
    } else {
  ...
  return (
    <>
      <div className="flex h-screen flex-col items-center justify-center overscroll-y-contain bg-gradient-to-br from-slate-700 to-slate-900 py-8 px-6 text-slate-600">
              {/* <h3 className="text-3xl text-slate-100 mb-2">Messages App</h3> */}
            ...
            <ul className="overflow-y-scroll px-3 pt-2">
              <AnimatePresence initial={false}>
                {[...messages].reverse().map((mid) => (
                  // vanilla css is not able to animate height from 0 to auto, but framer motion allows us to do this.
                  <motion.li
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: 'auto' }}
                    transition={{ opacity: { duration: 0.2 } }}
                    exit={{ opacity: 0, height: 0 }}
                    key={mid} className="relative">
                    <div className="py-0.5">
                      ...
                      >
                        <p className={`${selectedMessages.includes(mid) ? "text-white" : " text-slate-500"} truncate text-sm font-medium`}>
                          {titles[mid % titles.length][0]}
                        </p>
                        <p className={`${selectedMessages.includes(mid) ? "text-blue-400" : " text-slate-400"} truncate text-xs`}>
